...
HB_FUNC( WVT_SETTOOLTIPWIDTH )
{
   ...
   if ( ISNUM( 1 ) )
   {
      SendMessage( _s->hWndTT, TTM_SETMAXTIPWIDTH, 0, ( LPARAM ) ( int ) hb_parnint( 1 ) );
   }
...
HB_FUNC( WVT_SETMENU )
{
   ...
   SetMenu( _s->hWnd, ( HMENU ) hb_parnint( 1 ) ) ;
...
HB_FUNC( WVT_APPENDMENU )
{
  ...
  {
    lpszCaption = ( LPCTSTR ) hb_parnint( 4 ) ; // It is a SEPARATOR or Submenu
  }

  hb_retl( AppendMenu( ( HMENU ) hb_parnint( 1 ), ( UINT ) hb_parni( 2 ), ( HB_PTRDIFF ) hb_parnint( 3 ), ( LPCTSTR ) lpszCaption ) ) ;
}
...
{
  hb_retl( DeleteMenu( ( HMENU ) hb_parnint( 1 ), ( UINT ) hb_parni( 2 ), ( UINT ) hb_parni( 3 ) ) );
}
...
{
  hb_retl( DestroyMenu( ( HMENU ) hb_parnint( 1 ) ) );
}
...
{
   hb_retni( EnableMenuItem( ( HMENU ) hb_parnint( 1 ), ( UINT ) hb_parni( 2 ), ( UINT ) hb_parni( 3 ) ) );
}
...
HB_FUNC( WVT_TRACKPOPUPMENU )
{
   ...
   hb_retnl( TrackPopupMenu( ( HMENU ) hb_parnint( 1 ) ,
...
HB_FUNC( WVT_CREATEDIALOGMODAL )
{
   ...
   HWND     hParent   = ISNIL( 5 ) ? _s->hWnd : ( HWND ) hb_parnint( 5 );
...
HB_FUNC( WVT_LBADDSTRING )
{
   ...
   SendMessage( GetDlgItem( ( HWND ) hb_parnint( 1 ), hb_parni( 2 ) ), LB_ADDSTRING, 0, ( LPARAM )( LPSTR ) text );
   HB_TCHAR_FREE( text );
}
...
{
   hb_retnl( ( long ) SendMessage( GetDlgItem( ( HWND ) hb_parnint( 1 ), hb_parni( 2 ) ), LB_GETCOUNT, 0, 0 ) );
}
...
{
   SendMessage( GetDlgItem( ( HWND ) hb_parnint( 1 ), hb_parni( 2 ) ), LB_DELETESTRING, hb_parni( 3 ), 0 );
}
...
{
   SendMessage( GetDlgItem( ( HWND ) hb_parnint( 1 ), hb_parni( 2 ) ), LB_SETCURSEL, hb_parni( 3 ), 0 );
}
...
{
   LPTSTR text = HB_TCHAR_CONVTO( hb_parc( 3 ) );
   SendMessage( GetDlgItem( ( HWND ) hb_parnint( 1 ), hb_parni( 2 ) ), CB_ADDSTRING, 0, ( LPARAM )( LPSTR ) text );
   HB_TCHAR_FREE( text );
}
...
{
   SendMessage( GetDlgItem( ( HWND ) hb_parnint( 1 ), hb_parni( 2 ) ), CB_SETCURSEL, hb_parni( 3 ), 0 );
}
...
HB_FUNC( WVT_DLGSETICON )
{
   ...
   if ( hIcon )
   {
      SendMessage( ( HWND ) hb_parnint( 1 ), WM_SETICON, ICON_SMALL, ( LPARAM ) hIcon ); // Set Title Bar ICON
      SendMessage( ( HWND ) hb_parnint( 1 ), WM_SETICON, ICON_BIG,   ( LPARAM ) hIcon ); // Set Task List Icon
   }
...
HB_FUNC( WIN_SENDMESSAGE )
{
   ...
   hb_retnl( ( ULONG ) SendMessage( ( HWND ) hb_parnint( 1 ),
                                    ( UINT ) hb_parni( 2 ),
...
HB_FUNC( WIN_SENDDLGITEMMESSAGE )
{
   ...
   hb_retnl( (LONG) SendDlgItemMessage( (HWND) hb_parnint( 1 ) ,
                                        (int)  hb_parni( 2 ) ,
                                        (UINT) hb_parni( 3 ) ,
...
{
   hb_retl( SetTimer( ( HWND ) hb_parnint( 1 ), hb_parni( 2 ), hb_parni( 3 ), NULL ) != 0 );
}
...
{
   hb_retnl( ( ULONG ) SetTextColor( ( HDC ) hb_parnint( 1 ), ( COLORREF ) hb_parnl( 2 ) ) );
}
...
{
   hb_retnl( ( ULONG ) SetBkColor( ( HDC ) hb_parnint( 1 ), ( COLORREF ) hb_parnl( 2 ) ) );
}
...
{
   hb_retni( ( int ) SetBkMode( ( HDC ) hb_parnint( 1 ), hb_parni( 2 ) ) );
}
...
{
   hb_retnint( ( HB_PTRDIFF ) SelectObject( ( HDC ) hb_parnint( 1 ), ( HGDIOBJ ) hb_parnint( 2 ) ) );
}
...
HB_FUNC( WIN_SETMENU )
{
   SetMenu( ( HWND ) hb_parnint( 1 ), ( HMENU ) hb_parnint( 2 ) ) ;
}
...
HB_FUNC( WIN_SETDLGITEMTEXT )
{
   ...
   SetDlgItemText( ( HWND ) hb_parnint( 1 ), hb_parni( 2 ), lpBuffer );
...
{
   USHORT iLen = ( USHORT ) SendMessage( GetDlgItem( ( HWND ) hb_parnint( 1 ), hb_parni( 2 ) ), WM_GETTEXTLENGTH, 0, 0 ) + 1 ;
   LPTSTR cText = ( LPTSTR ) hb_xgrab( iLen * sizeof( TCHAR ) );
   char * szText;
   UINT iResult;

   iResult = GetDlgItemText( ( HWND ) hb_parnint( 1 ), // handle of dialog box
...
{
   hb_retl( CheckDlgButton( ( HWND ) hb_parnint( 1 ), hb_parni( 2 ),
...
{
   hb_retni( IsDlgButtonChecked( ( HWND ) hb_parnint( 1 ), hb_parni( 2 ) ) ) ;
}
...
{
    hb_retl( CheckRadioButton( ( HWND ) hb_parnint( 1 ), // handle of dialog box
...
{
   hb_retnint( ( HB_PTRDIFF ) GetDlgItem( ( HWND ) hb_parnint( 1 ), hb_parni( 2 ) ) );
}
...
{
   InvalidateRect( ( HWND ) hb_parnint( 1 ), NULL, TRUE );
}
...
HB_FUNC( WIN_GETCLIENTRECT )
{
   ...
   GetClientRect( ( HWND ) hb_parnint( 1 ), &rc );
...
{
   hb_retl( hb_wvt_DrawImage( ( HDC ) hb_parnint( 1 ), hb_parni( 2 ), hb_parni( 3 ),
...
HB_FUNC( WIN_GETDC )
{
   hb_retnint( ( HB_PTRDIFF ) GetDC( ( HWND ) hb_parnint( 1 ) ) );
}
...
{
   hb_retl( ReleaseDC( ( HWND ) hb_parnint( 1 ), ( HDC ) hb_parnint( 2 ) ) );
}
...
{
   Rectangle( ( HDC ) hb_parnint( 1 ), hb_parni( 2 ), hb_parni( 3 ), hb_parni( 4 ), hb_parni( 5 ) );
}
...
HB_FUNC( WIN_DRAWTEXT )
{
   ...
   hb_retl( DrawText( ( HDC ) hb_parnint( 1 ), lpBuffer, lstrlen( lpBuffer ), &rc, hb_parni( 4 ) ) );
   HB_TCHAR_FREE( lpBuffer );
}
...
{
   MoveWindow( (HWND) hb_parnint( 1 ), hb_parnl( 2 ), hb_parnl( 3 ), hb_parnl( 4 ), hb_parnl( 5 ), hb_parl( 6 ) );
}
...
{
   hb_retnint( ( HB_PTRDIFF ) SetParent( (HWND) hb_parnint( 1 ), (HWND) hb_parnint( 2 ) ) );
}
...
{
   hb_retl( IsWindow( (HWND) hb_parnint( 1 ) ) );
}
...
HB_FUNC( WIN_CLIENTTOSCREEN )
{
   ...
   {
      if ( ClientToScreen( (HWND) hb_parnint( 1 ), &Point ) )
      {
...
HB_FUNC( WIN_SCREENTOCLIENT )
{
   ...
   if ( wvt_Array2Point( pArray, &Point ) )
   {
      if( ScreenToClient( (HWND) hb_parnint( 1 ), &Point ) > 0 )
      {
          wvt_Point2ArrayEx( &Point, pArray );
          hb_retl( TRUE ) ;
      }
...
HB_FUNC( WVT__GETOPENFILENAME )
{
   ...
   ofn.hwndOwner        = ISNIL(1) ? GetActiveWindow() : (HWND) hb_parnint( 1 ) ;
...
{
   HMENU hMenu  = ( HMENU ) hb_parnint( 1 );
   UINT  uFlags = ISNIL( 2 ) ? TPM_CENTERALIGN | TPM_RETURNCMD : hb_parnl( 2 );
   HWND  hWnd   = ISNIL( 3 ) ? GetActiveWindow() : ( HWND ) hb_parnint( 3 );
...
HB_FUNC( WIN_CHOOSECOLOR )
{
   ...
   cc.lStructSize   = sizeof( CHOOSECOLOR ) ;
   cc.hwndOwner     = ISNIL( 4 ) ? NULL : (HWND) hb_parnint( 4 );
   cc.rgbResult     = ISNIL( 1 ) ?  0 : ( COLORREF ) hb_parnl( 1 ) ;
   cc.lpCustColors  = crCustClr ;
   cc.Flags         = ( WORD ) ( ISNIL( 3 ) ? CC_ANYCOLOR | CC_RGBINIT | CC_FULLOPEN : hb_parnl( 3 ) );
